...
bool strToFloat(const char* str, float* value);
	 ...
	 * \details
	 * This function tokenises a string into an array of tokens. The tokens are separated by the delimiters
	 * specified in the \p delimiters string. The tokens are stored in the \p tokens array and the number of
	 * tokens found is returned. The \p maxTokens parameter specifies the maximum number of tokens to store in
	 * the \p tokens array. If the \p tokens array is not large enough to store all of the tokens then the
	 * behaviour is undefined. The \p delimiters string is a C-string containing the characters to use for
	 * delimiting the tokens; e.g., " \t" will use space and tab characters as delimiters.
	*/
