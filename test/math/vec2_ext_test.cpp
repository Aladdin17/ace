...
TEST_CASE( "vec2_mult_matrix", "[vec2]" ) {
    SECTION( "nan arguments" ) {
        ac_vec2 nan_vec = ac_vec2_nan();
        ac_mat2 nan_mat = ac_mat2_nan();
        ac_vec2 vec = {1.0f, 1.0f};
        ac_mat2 mat = ac_mat2_identity();

        // vec is nan
        ac_vec2 result = ac_vec2_mult_matrix(&nan_vec, &mat);
        REQUIRE(ac_vec2_is_nan(&result) == true);

        // mat is nan
        result = ac_vec2_mult_matrix(&vec, &nan_mat);
        REQUIRE(ac_vec2_is_nan(&result) == true);
    }

    SECTION( "identity matrix" ) {
        ac_vec2 vec = {1.0f, 2.0f};
        ac_mat2 mat = ac_mat2_identity();
        ac_vec2 expected = vec;
        ac_vec2 result = ac_vec2_mult_matrix(&vec, &mat);
        REQUIRE(ac_vec2_is_equal(&result, &expected) == true);
    }

    SECTION( "zero matrix" ) {
        ac_vec2 vec = {1.0f, 2.0f};
        ac_mat2 mat = ac_mat2_zero();
        ac_vec2 expected = ac_vec2_zero();
        ac_vec2 result = ac_vec2_mult_matrix(&vec, &mat);
        REQUIRE(ac_vec2_is_equal(&result, &expected) == true);
    }
